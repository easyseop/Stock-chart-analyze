...
def test_happy_path_executes():
    ...
    print("[PASS] now·미보유·가격근접 → execute_entry 전송 + 캡 입력 + 손절선 기록")
...
def test_nyse_holding_merged():
    ...
    def pd(market="US", excg="NASD"):
        ...
        return []
    res, ex, rec = _run([_sig(code="KKR", ccy="USD", entry=90.0, stop=85.0)],
...
def test_earnings_window_skips():
    res, ex, rec = _run([_sig(earnings_d=2)], holdings={})
    assert _g(res, "005930")["gate"] == "earnings" and not ex.called
    res2, ex2, _ = _run([_sig(earnings_d=5)], holdings={})
...
def test_cooldown_after_same_day_sell():
    ...
    res, ex, rec = _run([_sig()], holdings={}, fold=fold)
...
def test_intra_cycle_accumulation():
    ...
    res, ex, rec = _run([_sig(code="005930"), _sig(code="000660")], holdings={})
