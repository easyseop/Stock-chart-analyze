...
`총보유 == 매도가능`이면 오탐 없음(P3로 강등), 다르면 확정 P2다.

**최소 수정**: 연속 N회(기본 2~3회) 같은 갭이 유지될 때만 알린다. 결제·일시
예약은 다음 사이클에 풀리고, INGR 같은 영구 누수만 남는다.
...
**P1-1 해소 후 재검토.** P2-1은 서버 실측(PEP·NKE)으로 등급을 확정한 뒤 같은
라운드에 처리하기를 권한다. P3-1은 테스트 1건이라 함께 넣으면 좋다.
