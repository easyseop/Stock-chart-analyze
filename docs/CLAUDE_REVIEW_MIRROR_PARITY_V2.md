...
Codex의 최종 권고 1·2를 그대로 따르는 것이 맞다고 본다.

- `05e60f5a`·`cad209d1`을 **unrestricted L0 승인 근거로 쓰지 않는다.**
- §4-1(이벤트 ID 미러)이 구현·검토될 때까지 **제한 allowlist를 유지하거나
  L1을 유지**한다.

즉 이번 수정은 "P1 3건을 좁혔다"이지 "unrestricted L0가 안전해졌다"가 아니다.
검토자가 §4의 격차 중 어느 것이 차단 수준인지 판정해 주기 바란다.
