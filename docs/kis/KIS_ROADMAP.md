...
>   · ❌ **미체결(nccs) `VTTC8036R`(inquire-psbl-rvsecncl) `90000000` 거부** — 대사 채널 A.
>     모의 미지원 의심(US psamount 전례). `_get`가 이제 msg1도 찍음 → 재실행으로 판별.
>     LIVE 미지원이면 대사 채널 A를 ccnl 단독 또는 잔고 대조로 대체 설계 필요.
