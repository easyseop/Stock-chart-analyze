...
>
> **국내 모의 왕복 2차 실측(2026-07-13, 하한가 매수) — 접수·취소 확정**:
>   · ✅ **매수 접수** `ok/ack` — 응답 output에 `ODNO`·`KRX_FWDG_ORD_ORGNO`(orgno) 실림.
>     → 원장 결속 필드명(`out.get("ODNO")`/`out.get("KRX_FWDG_ORD_ORGNO")`) **실측 확정**.
>   · ✅ **취소** `ok/canceled` — `VTTC0803U` + `KRX_FWDG_ORD_ORGNO` 전달 성공(실주문 취소됨).
>   · ✅ **체결내역(ccnl) `VTTC8001R` 작동**(rt_cd=0) — 미체결 취소건이라 0행(정상).
>     체결 행 구조는 `--fill`로 실측 필요(대사 채널 B 필드명).
>   · ❌ **미체결(nccs) `VTTC8036R`(inquire-psbl-rvsecncl) — 모의 미지원 확정**:
...
>   잔고 기반 별도 구현 필요. `kis_mock_roundtrip --fill`을 잔고 확인→원장 반영→청산으로
>   재설계(대사 채널 실측 겸용). 남은 실측: 잔고 output1 필드명(hldg_qty 등)·ccnl 체결 행.
...
## 요약 — 상태 (2026-07-13 갱신 · 국장 배선 + 국내 모의 실측 완료)
**장 없이 가능한 코드는 전부 완료**(A~C + **국내(KR) 전 경로**) — 테스트 **24개 모듈 green**.

**2026-07-13 국장(KR) 실행 계층 추가**(그동안 미국 해외주식만 배선돼 국장이 누락):
- ✅ 주문/취소/**시장가 손절**/세션게이트/사이징 국내 배선(심볼 자동 라우팅, 같은 appkey).
- ✅ **국내 모의 왕복 실측 완료** — 시세·주문접수·취소·ODNO/ORGNO 결속 서버 확인.
  nccs(VTTC8036R) 모의 미지원 확정, 잔고(VTTC8434R) 지원 확정.
- ✅ **국내 UNKNOWN 잔고 기반 대사**(`reconcile_unknowns_kr`) — 안전정리(초과매도 구조적
  봉쇄), 설계·스트레스·구현검증 워크플로우 3회로 검증. chase도 국내화(호가단위·orgno).

**남은 것은 전부 실측/실물/별개배선**:
1. 🧪 LIVE 소액 실측 — `hldg_qty` 당일결제 의미·nccs·통합증거금·종목상태 코드값
2. 👤 서버 VM 설치(infra/server/README) + 🔨 CF Worker dead-man↔서버 하트비트(발행경로 결정 필요)
3. 👤 봇 전용 실전 계좌+appkey → 4. Stage 2 Go/No-Go
5. 🔨 (Stage 3 전) costbook 확정체결 배선 — 現 open_cost=0이라 총량 게이트 미작동,
   다종목 stage에서 과투입 방지 위해 필요(現 stage 1.5/2는 1종목 캡으로 안전)
6. 🔨 (선택) sell_cap 하드 백스톱·WS 체결통보·주간거래·기업행위 감지
