...
6. **[추가 검토 의뢰] CVNA 원가 소급 기입** — 실측(2026-08-19): CVNA 74주가
   원장·costbook에 매수 기록 없이 존재했고(고아), 복원 후 브로커-진실 원가에
   잡히며 불변식(open_cost>SEED) L1이 발동했다(35,094,687 − costbook A
   28,453,824 = 6,640,863 = 74×$65.03×1380, 소수점 일치). 사용자가 SEED A를
   상향해 임시 해소했으나, **costbook에는 여전히 CVNA 매입원가가 없다.**
   이대로 CVNA가 매도되면 매도대금만 계상돼 봇 현금이 ~664만원 부풀고, 그
   부푼 현금이 사이징 분모로 쓰인다. 질문: (a) costbook에 소급 buy 기입이
   안전한가(멱등키·이벤트 순서·기존 대사와의 충돌), (b) 아니면 CVNA 매도 시
   proceeds를 계상하지 않는 예외가 나은가, (c) 제3안이 있는가. 회계 쓰기라
   Codex 판단을 요청한다.
