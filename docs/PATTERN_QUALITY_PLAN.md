...
## 1차 분위수 결과 (2026-07-12 · 캐시 40종목 · 신호 10,806건)

| 지표 | Q1→Q4 기대R | 판정 |
|---|---|---|
| **clear_space_R** | +0.091 → +0.145 → **+0.165** → (∞)+0.023 | **유한 구간 단조 개선 ✓** — 공간 좁을수록 나쁨(GPT 예측 적중). 단 **no_overhead(상방 저항 미검출, 표본 25%)가 최악**(+0.023R·승률 34%·MAE −1.12) → "저항 없음=유리" 가정 **데이터로 반박**. quality 산식의 no_overhead +1은 재고 대상 |
| **ATR14/60 수축** | +0.024 → +0.044 → +0.124 → **+0.234** | **단조인데 방향이 예측과 정반대** — 우리 저점권 전환에선 변동성 **확장**이 좋음(수축 베이스 = 죽은 흐름?). "수축<0.8=양호" 치트시트 통념 **반박**. 하드게이트로 넣었다면 성과를 깎을 뻔 |
| close_location | +0.116 / +0.108 / +0.128 / +0.072 | **평평(무변별)** — 우리 신호 풀에선 엣지 없음. 승격 탈락 |
| extension_ATR | +0.070 → +0.075 → +0.107 → **+0.173** | 단조인데 역시 역방향(확장=강한 thrust가 좋음). 단 추격 게이트는 기대R이 아니라 MAE 통제 목적이라 **행동 변경 금지** |

**결론: 오늘 승격 0건(규칙대로).** 이유 — ①단조가 나온 두 지표(수축·확장)는 방향이
통념과 반대라 **표본외(OOS)·kind별(transition vs normal) 재검증 전 채택 금지**
②clear_space는 no_overhead 재정의 필요 ③표본이 40종목·시간중첩 신호라 상관 표본.
**이 실험의 진짜 수확**: "기록→검증→승격" 규율이 작동함을 증명 — 치트시트 통념
2개(수축 좋음·공간무한 좋음)를 하드룰로 넣기 전에 데이터가 걸러냄.
