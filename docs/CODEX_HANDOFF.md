...
### 34. 2026-08-06 월별 성과 화면·B 승률·미국 종목 확대 검토

`codex/monthly-performance-b-universe-review`에서 개인 성과 화면에 최근 6개월
월별 성과를 추가했다. 월별 본지표는 입출금 영향을 제거한 계좌/전략 A/전략 B의
TWR이며, 주 지수 월 복리와 초과수익률(%p)을 같은 표에서 비교한다. 매도월 기준
실현손익·매도원가 대비 실현수익률·승률은 보조지표로 분리했고 현재 미실현 손익과
합산하지 않는다. 해당 월 중 하루라도 계좌/전략/지수 값이 빠지면 그 지표는 숫자로
확정하지 않고 `—`로 표시한다.

월 실현수익률의 정확한 분모를 위해 인증된 개인 거래이력 매도 행에 costbook의
`cost_closed_krw`를 추가했다. 공개 성과 API는 계속 퍼센트 전용이고, 신규 KIS
호출·주문 엔드포인트는 없다. 개인 거래이력 조회 상한은 200건에서 서버가 이미
허용하는 500건으로 늘렸다.

2026-08-06 Oracle 원장 실측 기준 전략 B 완료 거래는 SIG 1건이며 1승 0패,
승률 100%, 실현수익률 +6.03%, 실현손익 +96,606.90원이다. 다만 표본이 단 1건이고
해당 매도가가 복원 추정값이므로 B의 우수성을 결론낼 수 없다. 화면에도 승률과 함께
`완료 1건·추정가 1건`을 노출해 100% 숫자만 오해하지 않게 했다.

검증은 Node 순수계산 19/19, 거래이력 테스트, 사이트 안전경계 테스트, JS 문법,
`git diff --check`를 통과했다. 로컬 미리보기에서 데스크톱 1280px와 모바일 390px를
확인했고 페이지 전체 가로 넘침과 콘솔 오류가 없었다. 모바일 월별 표만 카드 안에서
좌우 스크롤된다.

미국 종목 확대 실측·권고는 `docs/US_UNIVERSE_DAILY_GROWTH_REVIEW.md`에 기록했다.
현재 유니버스는 미국 보통주 5,113 + 한국 303 = 5,416종목이고, 최신 Actions 캐시는
5,414종목(134.42 MiB)이라 이미 99.96% 수집됐다. 따라서 무작위 10개 추가는 하지
않는다. 향후 NASDAQ 공식 목록에서 새로 발견된 보통주만 하루 최대 10개 수집하고,
최소 이력·유동성·완전성 그림자 게이트를 통과한 뒤 스캐너에 승격하는 별도 작업을
권장한다. 데이터 수집 확대는 KIS 신규매수 한도 확대 승인이 아니다.

### 35. 2026-08-11 TAP 매도 거절 대사 개선(R0~R5) Claude V2 재검토 준비

사용자가 `docs/CODEX_SPEC_SELL_REJECT_RECONCILE.md`의 R0~R4와 추가된
R5 구현을 지시했다. 기존 더티 작업트리는 건드리지 않고,
`codex/sell-reject-reconcile`을 기본 브랜치 `c3949ace`위에 새로
구성했다. 아직 병합·Oracle 배포·운영 원장 mutation은 하지 않았다.

R0는 Oracle KIS mock에서 읽기 전용으로 완료했다. ODNO
`0000038291`은 2026-08-10 NYSE 당일 체결내역에 TAP 80주·체결
0주·미체결 80주로 존재했고 상태/거절 사유 문구는 빈 값이었다.
미체결은 0행, TAP 잔고는 80주 불변이지만 매도가능수량이 0이었다.
다른 NYSE 보유 15종목은 매도가능수량이 양수라 TAP만의 구조적
mock 매도 불가로 판단했다. 7일 조회는 15행 후 연속키가 남아
있어 해당 행을 놓쳤고, 접수일 단일 조회가 실체를 밝혔다.

현재 구현은 다음과 같다.

- R1: 미체결→접수일 체결내역→완전 잔고 순서로 모두 성공하고,
  600초 이상 ACK의 ODNO가 두 주문조회에 없으며 잔고가 불변일
  때만 `rejected`로 종결한다. `None`·형식 불신·연속키는 모두
  무변경이고, 잔고 모순은 경보 후 잠금을 유지한다.
- R2/R3: 대사 성공시각·연속 실패·사유를 0600 원자 상태파일에
  공유하고 6회 연속 실패만 1회 경보한다. 종결 행의
  `msg_cd/msg1`은 제어문자 제거·200자 제한으로 근거 이벤트에
  남기고, 부재 증명은 “사유 미상”으로 정직하게 표시한다.
- R4: 30분 초과 submitted/ack를 행별 1회 경보하고 해소 회복도
  1회 알린다. ops JSON은 심볼/금액 없이 `stuck_acks` 수량만 담는다.
- R5: time/btgt도 `#attempts+1` 파생 키를 쓰고, 과거 고정 키를
  attempts에 포함한다. 기본은 세션당 1회만 시도하고 실패 시
  상한 경보 1회 후 다음 거래소 세션까지 대기한다(미국은 뉴욕 날짜,
  한국은 KST 날짜). 매도가능 0처럼
  원장 전송 전에 거부된 시도도 영속 상태 카운터가 세어 폭주를 막는다.

신규 TAP 동일형 회귀, 조회 실패 5종, 599초 경계, ODNO 선행0,
잔고 모순, KR mock/live 폴백 격리, 실패 streak, ACK 방치, R5
세션 상한 테스트를 추가했다.

Claude 1차 검토는 P0 1·P1 2·P2 3으로 차단했다. 지적은 미국 거래소
오귀속 부재증명, 응답 헤더/메시지 연속페이지 누락, ownership 경계 누락,
근거 행수 오기록, 미국 세션을 KST로 계산, 공유 health와 로컬 게이트 혼합
위험이었다. 모두 보완했고 ACK 방치 래치도 프로세스 재시작에 견디도록
영속화했다. 자세한 판정·수정은
...
### 41. 2026-08-22 ACK 총보유/매도가능 단위 분리와 동결 직접증거 대사

기본 브랜치 `b7d9e3c5`에서 별도 clean worktree와
`codex/ack-unit-mismatch` 브랜치를 만들었다. 사용자 checkout의 기존 미커밋
변경은 건드리지 않았다. 기본 브랜치 병합·Oracle 배포·kill/env/운영 원장 변경은
하지 않았다.

C1은 SELL의 `hldg_before`가 매도가능 수량을 담아 총보유 delta 소비자와 어긋난
INGR 사고를 고쳤다. 한 KIS 잔고 응답에서 total과 sellable을 독립 파싱하고,
전송 상한은 sellable을 계속 쓰되 hldg_before만 total로 기록한다. total만 불신이면
None을 기록하면서 보호 SELL을 계속하고, sellable 불신은 기존처럼 주문 전 차단한다.
INGR 11/6 재현에서 미체결 delta=0·동결0, 5주 체결 delta=5·회계확정이 통과했다.

C2는 동결을 잔고/부재 추론 차단으로 유지하면서 원장 결속 exact ODNO 단일 체결만
예외로 허용했다. ownership armed, 사용자 baseline 배제, 동일 심볼 broker
non-terminal 정확히 1건을 모두 요구한다. 자동 direct 확정은 회계까지 진행하지만
동결은 스스로 풀지 않는다. 신규 `scripts/kis_ack_resolve.py`는 fresh KIS
미체결·체결·총보유를 모두 재조회하고 운영자 ack가 있을 때만 감사 event와 함께
적용한다. 조회 실패는 원장·동결 파일 바이트 변경 0건이며, terminal일 때만 동결을
원자 해제·재검증한다.

C3은 주문/취소 응답 msg_cd/msg1과 exact ODNO의 마지막 상태를 append-only meta에
보존한다. 같은 관측 반복은 추가 기록하지 않는다. 종결 사유는 명시 사유, 마지막
관측, 접수 응답 순으로 설명하고 Bearer·credential·실제 env secret·8자리 이상
식별자를 제거한다. 공개 ntfy category-only 계약은 그대로다. 텔레그램 `/진단`에
사용자 명시 승인 범위대로 종목·BUY/SELL·정화 메시지 최대 3건만 표시한다. 계좌·
ODNO·원장키·가격·수량은 제외하고 저장/전송 이중 정화와 HTML escape를 적용했다.

검증은 Python 전체 **73/73**, Node **19/19**, compileall, app.js 문법,
diff check가 통과했다. 핵심 방어 8종(hldg 단위·동결 direct·baseline·다중주문·
safe clamp·응답 기록·식별자 redaction·operator ack)을 각각 제거한 mutation은 모두
대응 테스트 exit 1로 KILLED됐다. `/진단` 전용 테스트도 최대 3건·시크릿/긴 식별자·
