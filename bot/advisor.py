    ...
    if stale:
        ...
        if (state.get("stale_notice_day") != cfg.today_kst()
            ...
            notify.send(f"⚠️ 신호가 {age:.0f}분째 낡음 — 매수/도달 제안을 일시 중단"
                        ...
                        critical=True)
        nb = na = 0
    elif not cfg.SUGGEST_ALERTS:
        # 사용자 요청(2026-07-12): 제안형 알림(매수 제안·도달) 기본 OFF —
        #   텔레그램은 자동매매의 실제 액션(예약/매수/손절 체결)과 보유 손절
        #   경보만. 웹 대시보드에는 계속 표시되므로 정보는 유지된다.
        print("제안 알림 OFF(ADVISOR_SUGGEST_ALERTS!=1) — 매수/도달 제안 생략")
        nb = na = 0
    else:
