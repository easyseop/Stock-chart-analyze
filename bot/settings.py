...
# 슬리브 B(매물대 반등)의 승인된 가격 계약. scanner/config.py와 값이 같아야
# 하며, KIS는 신호 메타를 믿지 않고 실제 발주가·손절·목표로 다시 계산한다.
SHELF_MIN_RR = 1.5
SHELF_MAX_STOP = 0.15
...
# 거래 성적(승률) 공개 요약 발행 토픽 — 금액·수량·종목 없이 건수·비율만.
TRADE_STATS_TOPIC = _os_shelf.environ.get(
    "NTFY_TRADE_STATS_TOPIC", "stock-trades-9b3c15d7f2")
