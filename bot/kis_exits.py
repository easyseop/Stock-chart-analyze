...
def decide(entry: float, stop0: float, cur_stop: float, price: float,
    ...
    """청산 판단(순수 함수) — [("sell", qty, 사유), ("raise", 새손절), ...].

    xs: {"half": bool, "high": float} (호출부가 갱신·저장).
    stop0=진입 시 손절선(R 계산용), cur_stop=현재 유효 손절선(래칫 반영).
    """
    acts: list[tuple] = []
    ...
    if not xs.get("half") and price >= entry + R:
        xs["half"] = True
        if qty >= 2:
            acts.append(("sell", qty // 2, "익절 +1R 절반"))
        if entry > cur_stop:
            acts.append(("raise", entry))
            cur_stop = entry
    # ② 트레일(절반익절 후) — 최고가 − 1.5R, 올리기만
    if xs.get("half"):
...
def manage(broker, held: dict, today: str) -> None:
    ...
    for code, p in list(held.items()):
        ...
        for act in decide(entry, stop0, cur_stop, price, qty, xs,
            ...
            if act[0] == "raise":
                ...
                _, sq, why = act
                key = f"xe:{code}:{'half' if '익절' in why else 'time'}:{rec.get('opened','')}"
